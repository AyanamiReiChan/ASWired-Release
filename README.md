...
ASWired **v1.0.10** 部署仓库，包含网站、Go 主控、管理 Agent，以及我们修改的 **Komari 1.2.5-fix2-aswired.1.0.10**。这里保存部署脚本、配置和文档；编译好的程序在 [Releases](https://github.com/AyanamiReiChan/ASWired-Release/releases) 下载，无需在服务器安装 Go、npm 或编译源码。
...
v1.0.10 默认使用均衡行为限速，新增宽松、均衡、严格预设与默认开启的代理业务 IPv6 屏蔽。已有自定义限速保留；已有节点的 IPv6 策略需要升级管理 Agent 并重新应用配置，客户端需更新订阅。见 [默认限速与代理 IPv6 策略](docs/DEFAULT-NETWORK-POLICY.md)。
...
主控健康检查应返回 `status=ok`、`version=v1.0.10`；Komari 版本应为 `1.2.5-fix2-aswired.1.0.10`。安装器不会修改系统防火墙、覆盖已有数据或生成管理员账户。
...
SQLite 正式组合部署可在「设置 → 系统维护」确认版本后直接升级主控、网站和 Komari，查看实际进度与结果。旧版本首次迁入需用脚本升级并启用更新服务。完整步骤见 [docs/UPGRADE.md](docs/UPGRADE.md)。主控、网站与修改版 Komari 随组合包一起更新；已有节点的 Agent 在页面手动升级，避免无意中断全部代理连接。
